...
tally_question <- function(df, question_name, rm.na = TRUE, sort = TRUE) {
  ...
  results_df1 <- filtered_df %>% 
    count(!!quoted_question, sort = sort) %>% add_tally(name = "nn")
...
## What is R Used For?  (English R Users Only)

```{r R Used For, echo = TRUE}
r_use_dictionary <- read_delim("dictionary/use_dictionary.tsv", delim = "\t", col_types = "cc")

# Breaks up uses into nested lists
uses <- english_survey %>% mutate(application = purrr::map(Qused_for, str_split, ", ")) %>% unnest()
apps_used <- uses %>% unnest() %>% select(application)
parsed_use_used <- apps_used %>% 
  left_join(r_use_dictionary, c("application" = "Input"))
parsed_use_exceptions <- apps_used %>% 
  anti_join(r_use_dictionary, c("application" = "Input"))
# Count the non-NA entries now
use_responses <- sum(!is.na(english_survey$Qused_for))
...
lollipop_chart(app_plot, 
               Output, 
               fill_color = params$bar_colors,
               subtitle_string =  question_text("Qused_for", "(English R users only)", 75),
               caption = paste0(survey_name, ", n = ", use_responses))
plot_save("What Do You Use R For (English R Users Only).pdf")

```

## What is R Used For?  (Spanish R Users Only)

```{r R Used For, echo = TRUE}
r_use_dictionary <- read_delim("dictionary/use_dictionary.tsv", delim = "\t", col_types = "cc")

# Breaks up uses into nested lists
uses <- spanish_survey %>% mutate(application = purrr::map(Qused_for, str_split, ", ")) %>% unnest()
apps_used <- uses %>% unnest() %>% select(application)
parsed_use_used <- apps_used %>% 
  left_join(r_use_dictionary, c("application" = "Input"))
parsed_use_exceptions <- apps_used %>% 
  anti_join(r_use_dictionary, c("application" = "Input"))
# Count the non-NA entries now
use_responses <- sum(!is.na(spanish_survey$Qused_for))
...
lollipop_chart(app_plot, 
               Output, 
               fill_color = params$bar_colors,
               subtitle_string =  question_text("Qused_for", "(Spanish R users only)", 75),
               caption = paste0(survey_name, ", n = ", use_responses))
plot_save("What Do You Use R For (Spanish R Users Only).pdf")
...
```
## English NPS versus Spanish NPS

## Net Promoter Score  (Spanish R Users Only)

```{r Net Promoter Score, echo = TRUE}

recommend_totals <-spanish_survey %>% group_by(Qrecommend) %>% tally()
responses <- sum(!is.na(spanish_survey$Qrecommend))
recommend_totals <- recommend_totals %>% mutate(percent = n / responses * 100)
recommend_scores <- tibble(Qrecommend = 1:10) %>% left_join(recommend_totals, by = "Qrecommend")
nps <- round(sum(recommend_scores$percent[9:10], na.rm=TRUE) - 
               sum(recommend_scores$percent[1:6], na.rm=TRUE))  # Standard NPS formula (see Wikipedia)
...
ggplot(recommend_scores) +
  ...
  geom_text(aes(x = Qrecommend, y = percent, label = paste0(round(percent), "%")), 
            color="black", size=3, nudge_y = 3) +
  scale_x_continuous(breaks = 1:10) +
    labs(title = question_text("Qrecommend", "(Spanish R users only)", 70),
       subtitle = sprintf("Net Promoter Score = %s,        Rating: %s", nps, npr),
       caption = paste0(survey_name, " (Spanish R Users Only), n = ", responses),
       x="Score", y="Count") +
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
        plot.background = element_rect(fill = "transparent",colour = NA))
plot_save("Net Promoter Score.pdf")
...
## Net Promoter Score  (English R Users Only)

```{r Net Promoter Score, echo = TRUE}

recommend_totals <-english_survey %>% group_by(Qrecommend) %>% tally()
responses <- sum(!is.na(english_survey$Qrecommend))
recommend_totals <- recommend_totals %>% mutate(percent = n / responses * 100)
recommend_scores <- tibble(Qrecommend = 1:10) %>% left_join(recommend_totals, by = "Qrecommend")
nps <- round(sum(recommend_scores$percent[9:10], na.rm=TRUE) - 
               sum(recommend_scores$percent[1:6], na.rm=TRUE))  # Standard NPS formula (see Wikipedia)
...
ggplot(recommend_scores) +
  ...
  geom_text(aes(x = Qrecommend, y = percent, label = paste0(round(percent), "%")), 
            color="black", size=3, nudge_y = 3) +
  scale_x_continuous(breaks = 1:10) +
    labs(title = question_text("Qrecommend", "(English R users only)", 70),
       subtitle = sprintf("Net Promoter Score = %s,        Rating: %s", nps, npr),
       caption = paste0(survey_name, " (English R Users Only), n = ", responses),
       x="Score", y="Count") +
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
        plot.background = element_rect(fill = "transparent",colour = NA))
plot_save("Net Promoter Score.pdf")
