...
# get the data
survey <- read_tsv(here::here(paste0("data/survey_", params$language,".tsv")),
...
ggplot(survey) +
  ...
  plot_save("Respondent Ages.pdf")
```

## Respondent Gender  (All Respondents)

```{r Respondent Gender, include = FALSE, echo = FALSE}
...
ggplot(data = plot_countries, aes(long, lat, group=group)) + 
  ...
  labs(title = "Geographical Distribution of Survey Respondents", 
       ...
       caption = survey_name,
       x = "", y = "", fill = "Responses")
  plot_save("Respondent Countries.pdf")
```
...
#   coord_polar("y") +
    ...
    theme(axis.line = element_blank(),
         axis.text = element_blank(),
         axis.ticks = element_blank())
  plot_save("Respondent Ethnicities.pdf")
```
...
r_years <- survey %>% select(Qr_year) %>% filter(Qr_year >= 1990)
mean_r_year <- round(mean(r_years$Qr_year, na.rm = TRUE))
...
ggplot(r_years) +
  ...
  geom_vline(aes(xintercept = mean_r_year), color = "lightblue") +
  annotate("text", x = mean_r_year-2.5, y = 400, label = paste0("Mean = ", round(mean_r_year, 0))) +
