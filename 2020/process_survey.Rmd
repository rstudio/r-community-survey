---
title: "2020 R Community Survey Results (Preliminary and Incomplete)"
...
- Survey was fielded between December 11, 2020 and January 08, 2021.
...
survey_name = "2020 R Community Survey"
...
tally_question_by_question <-
  function(df,
           question_name,
           by_question_name,     # column name to group by
           rm.na = TRUE,
           sort = TRUE) {
    quoted_question <- enquo(question_name)
...
## This version does a bar chart faceted by another variable

bar_chart_faceted <- function(df,
                      column_name,
                      facet_name,
                      fill_color,
                      title_string = "",
...
```{r read_processed_survey, include = FALSE}
# for plots
survey_name <- "2020 R Community Survey"
...
# get the data
survey_raw <- read_tsv("data/2020-combined-survey-final.tsv",
                       col_types = column_formats)

survey_questions <-
  read_tsv(here::here("data/2020-combined-survey-names.tsv")) %>% 
  select("Question_name" = english_name, "Question_text" = english)
# Saving some metadata for later
respondents_raw <- nrow(survey_raw)
survey_non_robot <- survey_raw %>% 
  mutate(robot_test = ifelse(!is.na(Qrobot_test), tolower(Qrobot_test), NA))
survey <- survey_non_robot %>%
  filter(!is.na(robot_test)) %>% 
  filter(robot_test == "5" | str_detect(robot_test, "five") | robot_test == "cinco" | robot_test == "fife")
...
write_csv(uncoded_genders, "dictionary/uncoded_genders_language.csv")  # For detecting open ends we should code
...
```{r Respondent Ages}
mean_age <- mean(2021 - survey$Qyear_born, na.rm = TRUE)

ggplot(survey) +
  geom_histogram(aes(2021 - Qyear_born), binwidth = 1,  fill = params$bar_colors, color = "white") +
...
```{r Respondent Gender, eval = FALSE}
...
country_map_countries <- country.map %>% count(region, sort = TRUE) %>% arrange(region)
cc_countries <- choro_countries %>% count(region, sort = TRUE) %>% arrange(region)
combined <- country_map_countries %>% left_join(cc_countries, by = "region")
exceptions <- country_map_countries %>% anti_join(cc_countries, by = "region")

plot_countries <- country.map %>% left_join(choro_countries, by = "region")
pc <- plot_countries %>% select(region, value, continent)

clist <- choro_countries %>% count(region, sort = TRUE)

ggplot(data = plot_countries, aes(long, lat, group = group)) + 
  geom_polygon(aes(fill = value), color = "gray", size = 0.1) +
  scale_fill_gradient(trans = "log10",  na.value = "gray80", low = "dark blue", high = "yellow") +
  coord_fixed(1.3) +
...
r_industry_exception_list <- r_industry_exceptions %>% 
  count(Qindustry, sort = TRUE) %>% 
  select(Qindustry, n)
write.csv(r_industry_exception_list, "dictionary/uncoded_industry_names.csv")
...
### Computer Languages or Tools You Are Comfortable With

```{r Tools Used Prior To Learning R}

languages <- survey %>% select(Qcoding_languages)
num_language_responses <- sum(!is.na(languages$Qcoding_languages))
languages_used <- split_and_aggregate(languages, Qcoding_languages)
...
lollipop_chart(top_10_items, 
          column_name = items, 
          fill_color = params$bar_colors, 
          subtitle_string = question_text("Qcoding_languages", "(All respondents)", 70),
...
```{r Age when respondents learned R}
...
```{r Number R Team Members, eval = FALSE}
...
```{r R Learning Ages By Learner Type, eval = FALSE}
...
plot_save("What Tools Do You Use With R (R Users Only).png")
```
...
### What Do You Use R For?  (R Users Only)
...
### Net Promoter Score for R

```{r R Net Promoter Score}
...
plot_save("Net Promoter Score for R.pdf")
```
...
```{r Frequency of Python Use}

### Need a python use dictionary. Here's how we derive that:
# python_use_responses <- survey %>% 
#   count(Qpython_use, sort=TRUE)
# write_tsv(python_use_responses, "dictionary/python_use_dictionary")
### And then we hand edit the mapping.

use_dictionary <- read_delim("dictionary/python_use_dictionary", delim = "\t", col_types = "cc")

python_use_responses <- survey %>% select(Qpython_use)
python_use_parsed <- python_use_responses %>% 
   left_join(use_dictionary, c("Qpython_use" = "Input"))
python_use_exceptions <- python_use_responses %>% 
  anti_join(use_dictionary, c("Qpython_use" = "Input"))
python_use_results <- tally_question(python_use_parsed, Output, rm.na = TRUE, sort = TRUE)
python_use_responses <- first(python_use_results$nn)

python_use_results <- python_use_results %>% 
 mutate(Output = factor(Output, levels = rev(Output)),
         position = cumsum(percent) - percent / 2,
         label = ifelse(percent >= 2, paste0(Output, "\n", round(percent), "%"), ""))

lollipop_chart(df                 = python_use_results,
          column_name        = Output,
          subtitle_string        = question_text("Qpython_use", ""),
          caption_string         = paste0(survey_name, ", n = ", python_use_responses),
          fill_color             = params$bar_colors)
plot_save("Frequency of Python Use.pdf")
plot_save("Frequency of Python Use.png")


```

### What Tools Do You Use With Python?

```{r Python Tools Used}

uses <- survey %>% transmute(tools = str_replace_all(Qpython_tools, "i.e., ", ""))
num_tool_responses <- sum(!is.na(survey$Qpython_tools))
tools_used <- split_and_aggregate(uses, tools)
tools_used <- tools_used %>% 
  select(-num_responses) %>% 
  mutate(percent = round(n / num_tool_responses * 100))
top_15_items <- top_n_choices(tools_used, items, num_tool_responses, 20) %>% arrange(percent)
top_15_items <- top_15_items %>%
  mutate(items = factor(as.character(items), levels = items))
...
lollipop_chart(top_15_items, 
          column_name = items, 
          fill_color = params$bar_colors, 
          subtitle_string = question_text("Qpython_tools", "(Python users only)", 60),
          caption_string = paste0(survey_name, ", n = ", num_tool_responses))
plot_save("What Tools Do You Use With Python (Python Users Only).pdf")
plot_save("What Tools Do You Use With Python (Python Users Only).png")
...
r_enjoyment <- tally_question(survey, Qpython_enjoy, rm.na = TRUE, sort = FALSE)
diff_responses <- first(r_enjoyment$nn)
mean_enjoyment = round(mean(survey$Qpython_enjoy, na.rm = TRUE), 1)

r_enjoyment <- r_enjoyment %>%
  mutate(percent = percent / 100)      # the plot below uses a percent y axis

ggplot(r_enjoyment) +
  geom_bar(aes(x = Qpython_enjoy, y = percent),
           stat = "identity",
           color = params$bar_colors,
           fill = params$bar_colors,
           width = 0.5,
           alpha = 0.5) +
  geom_text(aes(x = Qpython_enjoy, y = percent,
                label = paste0(round(percent * 100), "%")),
                color = "gray30",
                nudge_y = .03) +
  geom_vline(aes(xintercept = mean_enjoyment), color = "darkblue") +
  annotate("text", x = 4, y = 0.5, label = paste0("Mean value = ", mean_enjoyment)) +
  scale_y_continuous(labels = scales::percent) +
  labs(subtitle = question_text("Qpython_enjoy", "(Python users only)", 75),
       caption = paste0(survey_name, ", n = ", diff_responses),
       x="Respondent Rating (1 = Not at all, 5 = A great deal)", y="") +
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
        plot.background = element_rect(fill = "transparent",colour = NA))
plot_save("Enjoyment Using Python (Python Users Only).pdf")
```

### Net Promoter Score for Python

```{r Python Net Promoter Score}

recommend_totals <-survey %>% group_by(Qpython_recommend) %>% tally()
responses <- sum(!is.na(survey$Qpython_recommend))
recommend_totals <- recommend_totals %>% mutate(percent = n / responses * 100)
recommend_scores <- tibble(Qpython_recommend = 1:10) %>% left_join(recommend_totals, by = "Qpython_recommend")
nps <- round(sum(recommend_scores$percent[9:10], na.rm=TRUE) - 
               sum(recommend_scores$percent[1:6], na.rm=TRUE))  # Standard NPS formula (see Wikipedia)
npr <- nps %>% cut(breaks = c(-100, 0, 50, 75, 100), labels = c("Dangerous", "Good", "Excellent", "World Class"))

ggplot(recommend_scores) +
  geom_col(aes(x = Qpython_recommend, y = percent), fill = params$bar_colors) +
  geom_text(aes(x = Qpython_recommend, y = percent, label = paste0(round(percent), "%")), 
            color="black", size=3, nudge_y = 3) +
  scale_x_continuous(breaks = 1:10) +
    labs(title = question_text("Qpython_recommend", "(Python users only)", 35),
       subtitle = sprintf("Net Promoter Score = %s,        Rating: %s", nps, npr),
       caption = paste0(survey_name, " (R Users Only), n = ", responses),
       x="Score", y="") +
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
...
plot_save("Net Promoter Score for Python.pdf")
plot_save("Net Promoter Score for Python.png")
```
...
This plot is currently broken because the translation dictionary hasn't been updated
...
```{r Like Best About R, eval=FALSE}
...
```{r Like Least About R, eval=FALSE}
...
```{r Most Difficult Aspect of Learning}
...
community_sense <- survey %>% 
  count(Qr_community) %>% 
  rename("Input" = Qr_community, "Output" = n)
# write_tsv(community_sense, here::here("dictionary/community-dictionary.tsv"))

community_dictionary <- read_tsv("dictionary/community-dictionary.tsv", col_types = "cc")
community <- survey %>% 
  select(Qr_community) %>% 
  drop_na() %>% 
  left_join(community_dictionary, c("Qr_community" = "Input"))
community_exceptions <- survey %>% 
  select(Qr_community) %>% 
  drop_na() %>% 
  anti_join(community_dictionary, c("Qr_community" = "Input"))

community_results <- tally_question(community, Output, sort = FALSE) %>% 
  mutate(Value = ifelse(Output == "I'm not sure",
                        "Not Sure", 
                        str_extract(Output, "^.")))
mylevels <- levels(factor(community_dictionary$Output))

responses <- first(community_results$nn)
community_values <- community %>% 
  mutate(community_score = str_extract(Output, "^.")) %>%
  mutate(community_score_value = as.numeric(community_score))

mean_community = round(mean(community_values$community_score_value, na.rm = TRUE), 1)

community_results <- community_results %>%
  mutate(percent = percent / 100)      # the plot below uses a percent y axis

ggplot(community_results) +
  geom_bar(aes(x = Value, y = percent),
           stat = "identity",
           color = params$bar_colors,
           fill = params$bar_colors,
           width = 0.5,
           alpha = 0.5) +
  geom_text(aes(x = Value, y = percent,
                label = paste0(round(percent * 100), "%")),
                color = "gray30",
                nudge_y = .03) +
  geom_vline(aes(xintercept = mean_community), color = "darkblue") +
  annotate("text", x = 4, y = 0.5, label = paste0("Mean value = ", mean_community)) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +
  labs(subtitle = question_text("Qr_community", "", 75),
       caption = paste0(survey_name, ", n = ", diff_responses),
       x = "Respondent Rating (1 = No, not at all, 5 = Yes, definitely)", y = "") +
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
        plot.background = element_rect(fill = "transparent",colour = NA))
plot_save("R Community Membership (All Respondents).pdf")

```

### Community By Gender

```{r Community_by_gender}
...
community_dictionary <- read_tsv("dictionary/community-dictionary.tsv", col_types = "cc")
community <- survey %>% 
  select(Qr_community, Qgender_coded) %>% 
  drop_na() %>% 
  left_join(community_dictionary, c("Qr_community" = "Input"))
community_exceptions <- survey %>% 
  select(Qr_community) %>% 
...
faceted_results <- community %>% 
  count(Output, Qgender_coded)

totals <- group_by(Qgender_coded) %>% summarize(nn = sum(n), .groups = "drop_last")
...
ggplot(community_results) +
  ...
  theme(panel.background = element_rect(fill = "transparent",colour = NA),
        plot.background = element_rect(fill = "transparent",colour = NA))
plot_save("R Community Membership By Gender.pdf")
