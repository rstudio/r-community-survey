...
{
  ggsave(width = 6, height = 3.5, dpi = 600, units = "in", 
         here::here(paste0("plots/", params$language, "/", pathname)))
}

```


```{r read in survey, include = FALSE, eval = FALSE}
...
# the full text of the question. I've put short question names on another 
# sheet of the results Google Sheet, so  we do the substitution here.
...
ggplot(survey) +
  ...
  labs(subtitle = "Respondent Ages (All Respondents)", x="Age", y="Count")
  plot_save("Respondent Ages (All Respondents).pdf")
```
...
r_industry_dictionary <- read_delim("data/industry_dictionary.tsv", delim = "\t")
...
plot_save("Respondent Industries (All Respondents).pdf")
```
...
r_proficiency_dictionary <- read_delim("data/r_proficiency_dictionary.tsv", delim = "\t")
...
plot_save("Time To Achieve Proficiency In R (R Users Only).pdf")
...
r_why_dictionary <- read_delim("data/r_why_dictionary.tsv", delim = "\t")
r_why <- survey %>% select(Qr_reason_experienced) %>% 
  drop_na() %>% 
  left_join(r_why_dictionary, c("Qr_reason_experienced" = "Input"))
r_why_exceptions <- survey %>% select(Qr_reason_experienced) %>% 
  drop_na() %>% 
  anti_join(r_why_dictionary, c("Qr_reason_experienced" = "Input"))
r_why_results <- tally_question(r_why, Output, rm.na = TRUE, sort = TRUE)
r_why_responses <- first(r_why_results$nn)
...
r_experience_dictionary <- read_delim("data/r_experience_dictionary.tsv", delim = "\t")
...
lollipop_chart(df                 = r_experience,
          ...
          fill_color                 = params$bar_colors)
plot_save("R Experience (All Respondents).pdf")
...
r_tools_dictionary <- read_delim("data/tools_dictionary.tsv", delim = "\t")

# Breaks up uses into nested lists
uses <- survey %>% mutate(application = purrr::map(Qused_for, str_split, ", ")) %>% unnest()
tools_used <- uses %>% unnest() %>% select(application)
parsed_tools_used <- tools_used %>% 
  left_join(r_tools_dictionary, c("application" = "Input"))
parsed_tools_exceptions <- tools_used %>% 
  anti_join(r_tools_dictionary, c("application" = "Input"))
# Count the non-NA entries now
use_responses <- sum(!is.na(survey$Qused_for))

parsed_tools_tally <- parsed_tools_used %>% count(Output, sort = TRUE)

parsed_tools_tally <- parsed_tools_tally %>% 
  mutate(percent = round(n / use_responses * 100),
         prop_responses = n / use_responses) %>% 
  arrange(desc(percent))
app_plot <- head(parsed_tools_tally, 15) %>% 
  ungroup() %>% 
  arrange(prop_responses)

other <- tail(parsed_tools_tally, -15) %>% 
  ungroup() %>% 
...
difficulties_dictionary <- read_delim("data/difficulties_dictionary.tsv", delim = "\t")
...
use_dictionary <- read_delim("data/frequency_of_use_dictionary.tsv", delim = "\t")
...
pie_chart(df                 = tidyverse_use,
          ...
          reverse_fill_legend    = FALSE,
          colors                 = "Set1")
plot_save("Tidyverse Use Frequency Today (R Users Only).pdf")
```
...
aspects <- survey %>% 
  ...
  mutate(challenge = str_trunc(Qmost_difficult_aspect, 40))
r_difficult_aspect <- tally_question(aspects, challenge, rm.na = TRUE, sort = TRUE)
diff_responses <- first(r_difficult_aspect$nn)
r_difficult_aspect <-r_difficult_aspect %>%
  mutate(challenge = factor(as.character(challenge), levels = rev(challenge)))

bar_chart(r_difficult_aspect, 
          column_name = challenge, 
          fill_color = params$bar_colors, 
          title_string = "Most Difficult Aspect of Learning (Non-R Users)",
          subtitle_string = question_text("Qmost_difficult_aspect", 60),
...
time_to_success <- survey %>% 
  select(Qr_length_to_success)
r_expected_time_to_learn <- tally_question(time_to_success, 
                                           Qr_length_to_success, 
                                           rm.na = TRUE, 
                                           sort = TRUE) %>% 
  rename(learning_time = Qr_length_to_success)
time_responses <- first(r_expected_time_to_learn$nn)
r_expected_time_to_learn <- r_expected_time_to_learn %>%
  mutate(learning_time = factor(as.character(learning_time), levels = rev(learning_time)),
         position = cumsum(percent) - percent / 2,
         label = ifelse(percent >= 10, paste0(learning_time, "\n", round(percent), "%"), ""))

pie_chart(r_expected_time_to_learn, 
          column_name = learning_time,
          text_position = position,
          colors = "Set1", 
          title_string = "Expected Time To Success (Non-R Users)",
          subtitle_string = question_text("Qr_length_to_success", 60),
          caption_string = paste0(survey_name, ", n = ", diff_responses))
...
r_plan_dictionary <- read_delim("data/learning_plan_dictionary.tsv", delim="\t")
...
pie_chart(df                 = r_learning_plan,
          column_name        = Output,
          text_position          = 100 - position,
          title_string           = "Planned R Learning Path (Non-R Users)",
          subtitle_string        = question_text("Qhow_to_learn_r"),
...
ggplot(r_ages) +
  ...
  plot_save("Ages When Respondents Learned R.pdf")
```
