...
question_text <- function(question_name_string)
{
   ...
   if (length(question_text$Question_text) >= 55) {
...
survey <- survey %>% left_join(ethnicity_dictionary, by=c("Qethnicity_processed" = "Input"))
survey <- survey %>% mutate(Qethnicity_coded = ifelse(Qethnicity_coded == "Prefer not to answer", NA, Qethnicity_coded))
collected_ethnicities <- survey %>% group_by(Qethnicity_coded) %>% count(sort = TRUE)
write_csv(collected_ethnicities %>% select(Qethnicity_coded), "data/collected_ethnicities_english.csv")
...
ethnicities <- ethnicities %>% 
 mutate(Qethnicity_coded = factor(Qethnicity_coded, levels = Qethnicity_coded),
         position = cumsum(percent) - percent / 2,
...
# Plot
ggplot(r_experience) +
    geom_col(aes(x = 1, y = percent, fill = Qr_experience_factor), 
             width = 1, size = 1, color = "gray90", alpha = 0.4) +
    geom_text_repel(aes(x = 1.2, y = 100 - position, label = label),
              size = 3,
              hjust = 0.4,
    ...
    labs(x = NULL, y = NULL, 
         title = "R Experience",
         subtitle = question_text("Qr_experience"),
         caption = "2018 RStudio Learning R Survey",
