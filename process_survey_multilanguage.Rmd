...
r_proficiency <- r_proficiency %>% 
  mutate(Qtime_to_proficiency = factor(Qtime_to_proficiency, levels = rev(Qtime_to_proficiency)),
         position = cumsum(percent) - percent / 2,
         label = ifelse(percent >= 10, paste0(Qtime_to_proficiency, "\n", round(percent), "%"), ""))

pie_chart(df                 = r_proficiency,
          column_name        = Qtime_to_proficiency,
          text_position_expression = position,
...
ggplot(filter(survey, Qr_year >= 1990)) +
  geom_histogram(aes(Qr_year), binwidth = 1, fill = params$bar_colors, color="gray60") +
...
bar_chart(df                 = version_control_use,
          ...
          fill_color             = params$bar_colors)
ggsave("plots/Version Control Use (R Users Only).pdf")
```
...
bar_chart(df                 = unit_tests_use,
          ...
          fill_color             = params$bar_colors)
ggsave("plots/Unit Testing Use Frequency (R Users Only).pdf")
```
...
aspects <- survey %>% 
  ...
  mutate(challenge = str_trunc(Qmost_difficult_aspect, 30))
...
ggsave("plots/Most Difficult Aspect of Learning (Non-R Users).pdf")
...
pie_chart(r_expected_time_to_learn, 
          ...
          caption_string = paste0(survey_name, ", n = ", diff_responses))
ggsave("plots/Expected Time To Success (Non-R Users).pdf")
```
...
ggplot(r_difficulty) +
  ...
  labs(title = "Expected Difficulty in Learning R (Non-R Users)", 
       ...
       caption = paste0(survey_name, ", n = ", diff_responses),
       x="Respondent Rating (1 = Easiest, 5 = Most Difficult)", y="")
  ggsave("plots/Expected Difficulty in Learning R (Non-R Users).pdf")
```
