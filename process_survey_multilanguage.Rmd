...
ethnicities <- survey %>% transmute(categories = ifelse("," %in% Qethnicity, "Multi-ethnic", Qethnicity))
num_ethnicity_responses <- sum(!is.na(survey$Qethnicity))
ethnicities_found <- split_and_aggregate(ethnicities, categories)
ethnicities_found <- ethnicities_found %>% 
  select(-num_responses) %>% 
  mutate(percent = round(n / num_ethnicity_responses * 100))
top_15_items <- top_n_choices(ethnicities_found, items, num_tool_responses, 15)
