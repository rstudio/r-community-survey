...
survey <- read_delim(here::here("slides/data", paste0("final_survey_", language,".tsv")), col_types = survey_spec, delim="\t")
est <- locale(tz="US/Eastern")
survey$Qtime <- parse_datetime(survey$Qtime, format = "%m/%d/%Y %H:%M:%S", locale=est)
survey_questions <- read_delim(here::here("slides/data/final_survey_questions_English.tsv"), delim="\t")
...
ggplot(filter(survey, Qr_year >= 1990)) +
  geom_histogram(aes(Qr_year), binwidth = 1, fill = bar_color, color="black") +
