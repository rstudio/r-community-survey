...
## Plot a simple lollipop plot. The call form is a little baroque, so here's an example
## to illustrate the mapping of values to arguments.
...
lollipop_chart <- function(df,
  ...
  quoted_column <- enquo(column_name)
  nudge_amount <- max(df$percent, rm.na = TRUE) * 0.05
  p <- ggplot(df, aes(x = fct_reorder(Qtools_with_r, prop_tools), 
                      y = prop_tools)) +
    geom_point(aes(x = !!quoted_column, y = percent),
             fill =  fill_color) +
    geom_text(aes(x = !!quoted_column, y = percent , 
                  label = paste0(percent, "%")), 
              color="gray30", size=3, nudge_y = nudge_amount) +
...
used_for <- survey %>% 
  ...
  drop_na(Qused_fct) %>%
  mutate(prop_used = n / use_responses)

ggplot(used_for, aes(x = fct_reorder(Qused_fct, prop_used), 
                     y = prop_used)) +
  geom_col(fill =  params$bar_colors) +
...
ggplot(used_for, aes(x = fct_reorder(Qused_fct, prop_used), 
                     y = prop_used)) +
  geom_point(color = params$bar_colors, size = 2) + #<<
  geom_segment(aes(xend = fct_rev(Qused_fct), yend = 0), color = params$bar_colors) + #<<
  geom_text(aes(label = scales::percent(prop_used, accuracy = .1)), hjust = -.25, color="black", size=3) +
  labs(title = question_text("Qused_for"),
       caption = "2018 RStudio Learning R Survey",
       x="Application", 
       y="") +
...
use_tools <- survey %>% 
  summarize(responses = sum(!is.na(Qtools_with_r))) %>% 
  pull()

tools_used <- survey %>% 
  transmute(Qtools_with_r = str_replace_all(Qtools_with_r, "i.e., ", "")) %>% 
  separate_rows(Qtools_with_r, sep = ", ") %>% 
  mutate(Qtools_with_r = fct_lump(as.factor(Qtools_with_r), n = 15)) %>% 
  count(Qtools_with_r, sort = TRUE) %>% 
  drop_na(Qtools_with_r) %>%
  mutate(prop_tools = round(n / use_tools, 2))

ggplot(tools_used, aes(x = fct_reorder(Qtools_with_r, prop_tools), 
                     y = prop_tools)) +
  geom_point(color = params$bar_colors, size = 2) + #<<
  geom_segment(aes(xend = fct_rev(Qtools_with_r), yend = 0), color = params$bar_colors) + #<<
  geom_text(aes(label = scales::percent(prop_tools, accuracy = 1)), hjust = -.25, color="black", size=3) +
  labs(title = question_text("Qtools_with_r"),
       caption = "2018 RStudio Learning R Survey",
       x="Application", 
       y="") +
  coord_flip() +
