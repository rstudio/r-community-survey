...
tools_used <- survey %>% 
  ...
  mutate(Qtools_with_r = str_to_title(Qtools_with_r),
         Qtools_with_r = fct_lump(as.factor(Qtools_with_r), n = 15)) %>% 
  count(Qtools_with_r, sort = TRUE) %>% 
  drop_na(Qtools_with_r) %>%
  mutate(prop_tools = round(n / use_tools, 1))
...
ggplot(tools_used, aes(x = fct_reorder(Qtools_with_r, prop_tools), 
  ...
  geom_text(aes(label = scales::percent(prop_tools)), hjust = -.25, color="black", size=3) +
