...
chop_and_prop <- function(df, question_name, num_items = 15) {
  ...
  chops <- df %>% 
    ...
    #mutate(items = if_else(str_length(items) >= 4, str_to_title(items), items)) %>% 
...
lollipop_chart <- function(df,
  ...
  p <- ggplot(df, aes(x = fct_reorder(!!quoted_column, prop_responses), 
                      y = prop_responses)) +
    geom_point(color = fill_color, size = 2) +
    geom_segment(aes(xend = fct_rev(!!quoted_column), yend = 0), color = fill_color) +
...
```{r alison-r-used-for}
used_for <- chop_and_prop(survey, Qused_for, num_items = 14) 
